...
public class CommandManager implements org.bukkit.command.CommandExecutor {
	...
	private Map<String, CommandExecutor> commands; // Maps command names to
													// their executors
	...
	public boolean onCommand(final CommandSender sender,
		...
		if (args.length != 0) {
			if (this.commands.containsKey(args[0])) {
				runCommand(sender, commands.get(args[0]), Arrays.asList(args)
	...
	public void registerCommands(CommandExecutor[] executors) {
		for (CommandExecutor executor : executors) {
			String name = executor.getClass().getAnnotation(Command.class)
					.name();
			this.commands.put(name, executor);
		}
	}

	private void runCommand(CommandSender sender, CommandExecutor executor,
			List<String> args) {
		...
		} else if (args.size() < annotation.min()
				|| (annotation.max() != -1 && args.size() > annotation.max())) {
			sender.sendMessage(ChatColor.YELLOW + "Usage: /port "
